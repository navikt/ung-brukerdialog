...
# Generate ung-brukerdialog.openapi.json in the resources/openapi-ts-client directory
mvn --projects web exec:java

# Use docker to make the typescript client from the generated ung-brukerdialog.openapi.json
...
# Remove container from previous run if it exists
...
docker run \
  ...
  $IMAGE_URL -- \
  --openapi-spec-file in/ung-brukerdialog.openapi.json \
